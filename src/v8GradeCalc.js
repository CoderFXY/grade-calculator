...
const GradeCalculator = () => {
  ...
  const [subjects, setSubjects] = useState(() => {
    const stored = localStorage.getItem("gradeSubjects");
    try {
  ...
  const [targetGrades, setTargetGrades] = useState(() => {
    const stored = localStorage.getItem("targetGrades");
    try {
      return stored ? JSON.parse(stored) : {};
    } catch {
      return {};
  ...
  useEffect(() => {
    localStorage.setItem("gradeSubjects", JSON.stringify(subjects));
  }, [subjects]);
  
  useEffect(() => {
    localStorage.setItem("targetGrades", JSON.stringify(targetGrades));
  }, [targetGrades]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 p-4">
      ...
      <footer className="text-center mt-12 p-4 text-sm text-gray-500">
        © {new Date().getFullYear()} Grade Calculator. Built by IF(x) :)
      </footer>
    </div>
  );
};
