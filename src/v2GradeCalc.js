//Version 2 gradecalc (with target grades but formatting a bit weird because i want target grade to appear below the calculated average adjacent to text "needs to achieve...")
...
const GradeCalculator = () => {
  ...
  const calculateRequiredGrade = (subjectName) => {
    ...
    return requiredGrade <= 100 ? requiredGrade.toFixed(2) : null;
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {getSubjectAverages().map((sub) => (
              ...
              >
                ...
                {/* ADD HERE: target grade input */}
                <div className="mb-2">
                    <label className="block text-sm text-gray-600">Target Grade (%)</label>
                    <input
                        type="number"
                        min="0"
                        max="100"
                        ...
                        className="mt-1 w-full px-3 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm"
                    />
                </div>
                ...
                {targetGrades[sub.subject] && calculateRequiredGrade(sub.subject) && (
                    <p className="text-sm text-gray-500 mt-1">
                        Needs{" "}
                        <span className="font-semibold text-indigo-600">
                            {calculateRequiredGrade(sub.subject)}%
                        </span>{" "}
                        in remaining assessments to reach target.
                    </p>
                )}
